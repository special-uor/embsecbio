---
title: "EC-S: Updates 2021-03 - EMBSeCBIO"
...
## Modify EMBSeCBIO 
Modify the `entity_pub` table, to use an autonumeric in the column `ID_ENTITY_PUB`
```{sql, eval = FALSE}
...
```{r}
embsecbio::commit(conn, file.path(path, "Adding_Lake_Van_Pickarski et al. 2015.xlsx"), coerce = TRUE)
```
...
```{r}
dates_tb <- readxl::read_xlsx(file.path(path, "Dated_ages_for_38_entities_March2021.xlsx")) %>%
  magrittr::set_names(tolower(colnames(.)))
entities <- unique(dates_tb$entity_name)

for (e in entities) {
  embsecbio:::msg(e)
  tmp <- dates_tb %>%
    dplyr::filter(entity_name == e) %>%
    dplyr::rename(ID_ENTITY = id_entity) %>%
    dplyr::select(-entity_name)
  embsecbio:::update_records(conn, "date_info", tmp)
}
```
